...
struct Sam_Alignment *allocateMemorySam_Alignment(unsigned int max_read_length)
{
	...
	s->number_of_repetitions_perfect_match = 0;
	s->next_index_on_the_pool = -1;
	return s;
}
	...
	unsigned short int flag_ignore_quality_scores_for_matched_bases)
{
	...
	dest->number_of_repetitions_perfect_match = 0;
	dest->next_index_on_the_pool = -1;
}
								  ...
								  unsigned short int flag_ignore_quality_scores_for_matched_bases)
{
	...
	convertUnsignedIntegerToString(str, aln->core.flag);
	strcpy(dest->samflag, str);
